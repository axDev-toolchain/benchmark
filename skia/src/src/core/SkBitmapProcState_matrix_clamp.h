...
static void SCALE_NOFILTER_NAME(const SkBitmapProcState& s,
	...
	if (count >= 8) {
	    ...
	    do {
            ...
            int16x8_t v1 = vreinterpretq_s16_s32(lout);
            int16x8_t v2 = vreinterpretq_s16_s32(hout);
            int16x8x2_t vec_array = vuzpq_s16(v1, v2);
            lout = vreinterpretq_s32_s16(vec_array.val[0]);
            hout = vreinterpretq_s32_s16(vec_array.val[1]);
#else
...
static void PERSP_NOFILTER_NAME(const SkBitmapProcState& s,
	...
	// 2009/10/9: reworked to avoid illegal (but allowed by gas) insn
		...
		{
#ifdef __ARMCC_VERSION
                    int16x4x4_t buffer = vld4_s16(mysrc);
                    xlo = buffer.val[0];
                    xhi = buffer.val[1];
                    ylo = buffer.val[2];
                    yhi = buffer.val[3];
		...
		{
#ifdef __ARMCC_VERSION
                    int16x4x4_t buffer = vld4_s16(mysrc+16);
                    x2lo = buffer.val[0];
                    x2hi = buffer.val[1];
                    y2lo = buffer.val[2];
                    y2hi = buffer.val[3];
...
static void SCALE_FILTER_NAME(const SkBitmapProcState& s,
    ...
    #endif
#ifdef __ARMCC_VERSION
            wide_fx = vaddq_s32(wide_fx, vdupq_n_s32(dx+dx+dx+dx));
#else
...
static void AFFINE_FILTER_NAME(const SkBitmapProcState& s,
    ...
    #undef	AFFINE_DEBUG
		...
		t_x = wide_x; t_y = wide_y;
            ...
            fx += dx+dx+dx+dx;
#ifdef __ARMCC_VERSION
            wide_fy = vaddq_s32(wide_fy, vdupq_n_s32(dy+dy+dy+dy));
#else
...
static void PERSP_FILTER_NAME(const SkBitmapProcState& s,
    ...
    while ((count = iter.next()) != 0) {
        ...
        if (count >= 4) {
            ...
            while (count >= 4) {
#ifdef __ARMCC_VERSION
                int32x4x2_t buffer = vld2q_s32(srcXY);
                wide_x = buffer.val[0];
                wide_y = buffer.val[1];
#else
