...
static void SCALE_NOFILTER_NAME(const SkBitmapProcState& s,
	...
	if (count >= 8) {
		...
		/* this is ok -- we want all hi(lout)s then all hi(hout)s */
                ...
                int16x8_t tmp1 = vreinterpretq_s16_s32(lout);
                int16x8_t tmp2 = vreinterpretq_s16_s32(hout);
                int16x8x2_t vec_array = vuzpq_s16(tmp1, tmp2);
...
static void PERSP_NOFILTER_NAME(const SkBitmapProcState& s,
	...
	if (count >= 8) {
		...
		{
#ifdef __ARMCC_VERSION
                    int32x4x2_t buffer = vld2q_s32(mysrc);
                    x = buffer.val[0];
                    y = buffer.val[1];
#else
		...
		{
#ifdef __ARMCC_VERSION
                    int32x4x2_t buffer = vld2q_s32(mysrc+8);
                    x = buffer.val[0];
                    y = buffer.val[1];
#else
