...
function getDEAdHeadCookie( imageRef ) {
	...
	if ( !alreadySwappedDETargetImage && !cnnDEadDEonCookie) {
		...
		if(cnnU_Country && (cnnU_Country != cnnAdDE_UVal) && (cnnU_Country != '-')) {
			...
			if(cnnOn_Dom_Flag) { document.getElementById('cnnsetPref_Form').edition[1].checked = true; }
			else { document.getElementById('cnnsetPref_Form').edition[0].checked = true; }
		}
		else {
			CNN_setCookie('SelectedEdition', 'www', 854400, '/', '.cnn.com');
			if(cnnOn_Dom_Flag) { document.getElementById('cnnsetPref_Form').edition[0].checked = true; }
			else { document.getElementById('cnnsetPref_Form').edition[1].checked = true; }
		}
